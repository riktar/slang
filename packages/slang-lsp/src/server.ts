...
connection.onCompletion((params: TextDocumentPositionParams): CompletionItem[] => {
  ...
  if (/^\s*$/.test(lineText) || /^\s*\w*$/.test(lineText)) {
    ...
    const keywords = [
      ...
      { label: "deliver", kind: CompletionItemKind.Keyword, detail: "Post-termination handler" },
...
connection.onHover((params: TextDocumentPositionParams): Hover | null => {
  ...
  const keywordDocs: Record<string, string> = {
    ...
    budget: "**budget** — Set resource limits.\n\n`budget: tokens(N), rounds(N), time(N)` or `budget: time(Ns)`",
    deliver: "**deliver** — Post-termination side effect.\n\n`deliver: handler(args)`",
